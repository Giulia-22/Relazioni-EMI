...
\\[2pt]Dato che $g_m R_C\gg 1$, il circuito ha un guadagno in tensione, come ci aspettavamo. Il guadagno dipende dalla resistenza $R_C$ e dalla transconduttanza $g_m$. Quest'ultima introduce però alcuni problemi, perché ricordando che la transconduttanza è definita come:
...
\begin{figure}[h]
\centering
\includegraphics[height=10cm]{immagini/CEv2_pl}
...
\\Dato che $V_{CB}>0V$, la giunzione base-collettore è polarizzata inversamente, quindi l'ipotesi che il transistor si trovi in regione attiva diretta è verificata. 
\\Ora che asbbiamo risolto il circuito, calcoliamo anche la transconduttanza:
...
\\\indent Il guadagno dipende dal rapporto fra la resistenza di collettore e la resistenza di emettitore: se $R_C<R_E$ il circuito attenua l'uscita, quindi otteniamo un attenuatore; se $R_C>R_E$ l'uscita viene amplificata, perciò il circuito è un amplificatore; infine, se $R_C=R_E$ il circuito ha guadagno unitario, di conseguenza si comporta come un buffer. Nel nostro circuito, ci aspettiamo un guadagno, in modulo, di:
...
\begin{figure}[h]
\centering
\includegraphics[height=9cm]{immagini/fotoCEv2_pl}
...
\item oscilloscopio a due canali.
\end{itemize}
Per prima cosa, con il multimetro si sono misurati i valori delle resistenze. I valori ottenuti sono mostrati in tabella \ref{table:CEv2_comp}.
\begin{table}[h]
	\centering
	\begin{tabular}{|c|c|c|}
	\cline{2-3} 
...
\label{table:EFv1_pl_mis}
\end{table}
\\I valori ottenuti sono confrontabili con i risultati teorici calcolati nella sezione \ref{ptolavCEv2}. Come succede anche per i circuiti già analizzati, sia $V_B$ che $I_B$ non sono nulle, ma il loro valore (in modulo) è molto piccolo, perciò nel punto di lavoro possiamo trascurarle.
